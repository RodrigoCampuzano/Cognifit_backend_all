...
class CreateSpeiPaymentUseCase:
    """Checkout por transferencia SPEI. Igual que efectivo, la orden queda
    'pending' al responder: solo se sabe que se pagó cuando llega el webhook
    order.paid (ver handle_conekta_webhook.py), después de que el ADMIN
    transfiere desde su banco a la CLABE devuelta aquí."""
    ...
    async def execute(
        ...
        *,
        ...
        order = await self.gateway.create_spei_order(
        ...
        return await self.repository.attach_conekta_order(
            ...
            status="pending",
            spei_clabe=spei.get("clabe"),
            spei_bank=spei.get("bank"),
            spei_expires_at=_parse_expires_at(spei.get("expires_at")),
            raw_event=order["raw"],
        )
